...
//*****************************************
void packet_SEND(byte sbn, String data, int wait) { byte PROTOCOL_VERSION=1;
if (debugON>0) {Serial.print(F("\npacket_SEND, data="));Serial.println(data);Serial.flush(); }
  if (data!="NULL") {
    sMSG= "1|"; sMSG+=TX_ID;
...
String msg_GET(char *buf, byte len,String &key) {
  sRET="";
    if (debugON>0) {Serial.print(F("msg GET: "));print_HEX(buf,len); }
    if (debugON>0) {Serial.print(F("decoding with: ")); Serial.println(key);Serial.flush();}
  char *msg=decode(buf,len,key); msg[len]=0;
    if (debugON>0) {Serial.print(F("msg dec: ")); Serial.println(String(msg));Serial.flush(); }
  byte CSB=csb_MAKE(msg,len-1);
  byte csb=msg[len-1];
    if (debugON>0) {Serial.print(F("CSB: ")); Serial.print(CSB,HEX); }
    if (debugON>0) {Serial.print(F(", csb: ")); Serial.println(csb,HEX);Serial.flush(); }
    if (debugON>0) {print_HEX(msg,len-1);print_CHR(msg,len-1);}
...
void msg_SEND(String &msgIN, String &key, int txPWR) {
    if (debugON>0) {Serial.print(F("\nmsg_SEND: "));Serial.println(msgIN); }
  byte VBS = digitalRead(pinBOOST); if (VBS == 0) { boost_ON(); }
  char buf[64]; byte len=msgIN.length(); msgIN.toCharArray(buf,len+1);
    if (debugON>0) {print_CHR(buf,len);}
    if (debugON>0) { Serial.print(F("msg1 RF95_UP="));Serial.println(RF95_UP);}
  if (RF95_UP==false) { RF95_UP=init_RF95(); }
    if (debugON>0) { Serial.print(F("msg2 RF95_UP="));Serial.println(RF95_UP); }
  
  byte CSB=csb_MAKE(buf,len); 
  buf[len]=CSB;
    if (debugON>0) {Serial.print(F("CSB=")); Serial.print(CSB,HEX);}
    if (debugON>0) {Serial.print(F(", buf[")); Serial.print(len);Serial.print(F("]=")); Serial.print(buf[len],HEX);}
    if (debugON>0) {Serial.print(F(", key: ")); Serial.println(key);Serial.flush();}
  len++;  
  ...
  //if (VBS == 0) {  boost_OFF(); }
    if (debugON>0) {Serial.println(F("EO_msg_SEND")); Serial.flush();}
}

//*****************************************
//encode is just before sending, so is char array for rf95.send
char *encode(char *msg, byte len, String &key) {
...
//*****************************************
char *decode(char *msg, byte len, String &key) {
...
//*****************************************
byte csb_MAKE(byte *pkt, byte len) {  byte CSB = 0;
