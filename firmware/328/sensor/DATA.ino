...
char *get_TILT(char *data) { char *ret=data;
  ...
  byte optWords=(optBYTE & 0x0E)>>1; //0-7
  switch (optWords) {
    case 1: {if (strcmp(data,"LOW")==0) {strcpy(data,"HORZ");} else {strcpy(data,"VERT");} } break;
    case 2: {if (strcmp(data,"LOW")==0) {strcpy(data,"OPEN");} else {strcpy(data,"CLOSE");} } break;
    case 3: {if (strcmp(data,"LOW")==0) {strcpy(data,"LEFT");} else {strcpy(data,"RIGHT");} } break;
    case 4: {if (strcmp(data,"LOW")==0) {strcpy(data,"UP");} else {strcpy(data,"DOWN");} } break;
  }
  return ret;
}
